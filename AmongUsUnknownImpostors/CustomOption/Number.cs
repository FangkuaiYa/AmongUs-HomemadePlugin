...
namespace AmongUsUnknownImpostors.CustomOption
{
    ...
    {
        public CustomNumberOption(int id, string color, string name, float value, float min, float max, float increment, Func<object, string> format = null) : base(id, color, name, CustomOptionType.Number, value, format)
        {
            Min = min;
            Max = max;
            Increment = increment;
